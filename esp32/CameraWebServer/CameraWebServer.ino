...
void streamTask(void* parameter) {
  ...
  for (;;) {
    // Yeni client var mı?
    if (xQueueReceive(clientQueue, &client, 0) == pdTRUE && client != NULL) {
      ...
      while (client->connected()) {
        if (xSemaphoreTake(streamMutex, pdMS_TO_TICKS(500)) == pdTRUE) {
          ...
          if (fb) {
            ...
            size_t net_len = 0;
            while (net_len < fb->len) {
              size_t chunk_size = fb->len - net_len;
              if (chunk_size > 4096) chunk_size = 4096;
              
              if (client->write(fb->buf + net_len, chunk_size) != chunk_size) {
                break; // Yazma hatası
              }
              net_len += chunk_size;
      ...
      Serial.println("Stream client bağlantısı kapandı");
      client->stop();
      delete client;
      client = NULL;
    }
    
    vTaskDelay(pdMS_TO_TICKS(10)); // CPU yükünü azalt
  }
}
    ...
    *client = server.available();
    ...
    if (client->connected()) {
      ...
      // Stream istekleri
        ...
        xQueueSend(clientQueue, &client, 0);
